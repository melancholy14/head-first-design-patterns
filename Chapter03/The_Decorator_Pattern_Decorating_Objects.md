...
1. DarkRoast 객체에서 시작해볼까요.
2. 고객이 모카를 주문했으니까 Mocha 객체를 만들고 그 객체로 DarkRoast를 감쌉니다.
3. 고객이 휘핑크림도 추가했으니까 Whip 데코레이터를 만들어 Mocha를 감쌉니다.
4. 가격을 계산할 때는 가장 바깥쪽에 있는 Whip의 cost()를 호출하면 됩니다. 그러면 Whip은 Mocha의 cost()에게 가격 계산을 위임하고 리턴되면 휘핑크림의 가격을 더해서 그 결과값을 리턴합니다.
